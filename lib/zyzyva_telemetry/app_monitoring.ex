defmodule ZyzyvaTelemetry.AppMonitoring do
  ...
  def check_memory do
    ...
    status = cond do
      memory_mb > 4096 -> :critical  # > 4GB
      memory_mb > 2048 -> :warning   # > 2GB
      true -> :ok
    end
  ...
  defp get_from_loaded_apps do
    ...
    |> Enum.find_value(fn {app, _desc, _vsn} ->
      ...
      system_apps = [
        :kernel, :stdlib, :elixir, :logger, :iex, :mix, :ex_unit,
        :compiler, :crypto, :ssl, :public_key, :asn1, :inets,
        # Database/monitoring deps
        :telemetry, :telemetry_poller, :telemetry_metrics,
        :exqlite, :db_connection, :ecto, :ecto_sql, :postgrex,
        # Phoenix deps
        :phoenix, :phoenix_html, :phoenix_live_view, :phoenix_pubsub,
        :plug, :plug_crypto, :mime, :phoenix_ecto,
        # Common libraries
        :decimal, :ranch, :cowboy, :bandit, :thousand_island,
        # Our monitoring lib
        :zyzyva_telemetry
      ]
  ...
  defp build_health_check_fn(repo, extra_checks) do
    fn ->
      ...
      extra_results = Enum.map(extra_checks, fn {key, value} ->
        result = if is_function(value, 0) do
          value.()
        else
          value
        end
